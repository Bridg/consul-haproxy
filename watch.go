...
func buildTemplate(conf *Config,
	...
	// Create the template
	templ, err := template.New("output").Funcs(funcMap).Parse(string(raw))
	if err != nil {
...
//get the tag portion of given string generated by formatOutput() method above
func parseTag(t string) string {
	r, _ := regexp.Compile(`_\[(\w+)\]\s[\d\.:]+$`)
    result := r.FindStringSubmatch(t)
    if (result != nil) { return result[len(result)-1] }
    return "notag"
}
